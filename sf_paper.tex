...
Figure~\ref{fig:zth_curves} shows the averaged $Z_\text{th}(t)$ curve
and its cumulative structure function for the Schmidt Sciences H100~SXM5.

\begin{figure*}[tb]
  \centering
  \includegraphics[width=\textwidth]{figs/fig_zth_curves.pdf}
  \caption{%
    Left: transient thermal impedance $Z_\text{th}(t)$ for the Schmidt Sciences
    H100~SXM5, averaged over $n{=}5$ independent runs (bold) with individual
    runs shown faintly; averaging over runs reduces the effective 1\,\textdegree C
    quantization noise by $\sqrt{n}$.
    Right: cumulative structure function in resistance--capacitance space,
    derived via $R(t) = Z_\text{th}(t)$, $C(t) = t/Z_\text{th}(t)$, and
    plotted to 600\,J/K (the curve extends further as the liquid-cooling loop
    charges).
    The steep segment at $C \lesssim 100$\,J/K represents the low-capacitance,
    high-resistance die and package layers; the shallower rightward extension
    is the large-mass direct-liquid cooling path.
    A GPU with increased package-level thermal resistance would show a taller
    steep segment, lifting the entire curve and increasing $Z_\text{th,max}$.
  }
...
Averaging $N$ independent runs on the same GPU before computing the
structure function reduces the effective quantization noise by $\sqrt{N}$
and is recommended for structure-function visualization; the plateau
$Z_\text{th,max}$ used for fleet flagging is robust after a single run.
...
The elevated $Z_\text{th,max}$ indicates increased package-level thermal
resistance; physical inspection would be required to determine whether
the cause is dried TIM, a loose heatsink mount, or restricted slot
airflow. The elevation is entirely masked by the cooler ambient in raw
temperature logs.
...
\paragraph{Inter-cluster portability.}
The H100~SXM5 is the only GPU type measured on two independent
clusters, allowing a direct concordance check.
Sherlock yields $0.030 \pm 0.002$\,K/W across ten runs on multiple
nodes; Schmidt yields $0.036 \pm 0.004$\,K/W across five runs all on
node~g0378.
The 20\% offset is statistically significant (approximately 3$\sigma$
above the Sherlock mean) and likely reflects facility-level
differences: server chassis design, direct-liquid-cooling supply
temperature, and the fact that all Schmidt measurements come from a
single node rather than a spread of nodes.
Crucially, if the Sherlock baseline were applied directly to Schmidt,
the healthy Schmidt GPUs would sit at $1.2\times$ the reference
median and would approach the WARN threshold without any real
degradation.
This confirms that per-cluster re-calibration is necessary before
deploying thresholds, even for identical GPU models, and that
$Z_\text{th,max}$ captures facility-level cooling variation as well
as package-level degradation.
